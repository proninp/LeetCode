 ...
 * they need to decide which department is their primary department. Note that when an employee belongs
 * to only one department, their primary column is 'N'.
...
FROM 
       (
              SELECT  employee_id
                      , department_id
                      , primary_flag
                      , RANK() OVER(PARTITION BY employee_id ORDER BY primary_flag DESC) n
              FROM Employee 
       ) q
WHERE n = 1